...
export default function Benefits() {
    ...
    const { isPremium, FREE_BENEFITS_COUNT } = usePremium();
        ...
        <div className="min-h-screen pt-8 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                {benefitCategories.map((category, index) => {
                    const isLocked = index >= FREE_BENEFITS_COUNT && !isPremium;

                    if (isLocked) {
                        return (
                            <div key={category.id} onClick={() => setShowPaywall(true)} className="cursor-pointer">
                                <motion.div
                                    initial={{ opacity: 0, y: 20 }}
                                    animate={{ opacity: 1, y: 0 }}
                                    transition={{ delay: index * 0.05 + 0.1 }}
                                    className={`h-full min-h-[220px] rounded-3xl p-6 bg-gradient-to-br ${category.color} shadow-xl flex flex-col justify-end border border-white/10 relative overflow-hidden opacity-60`}
                                >
                                    ...
                                    <div className="absolute top-6 left-6 text-4xl opacity-40 relative z-10">{category.icon}</div>
                                    <div className="mt-auto opacity-40">
                                        <h3 className="text-2xl font-bold mb-1 text-black leading-tight">{getCatLabel(category.id, 'title') || category.title}</h3>
                                    </div>
                                </motion.div>
                            </div>
                        );
