    ...
    if (!fruit) {
        return (
            <div className="min-h-screen flex items-center justify-center">
                ...
                <button onClick={() => navigate(-1)} className="mt-4 text-fruit-green underline">Go Home</button>
            </div>
        );
    }
        ...
        <div className="pb-16 bg-fruit-light/60 dark:bg-fruit-dark/50 min-h-[calc(100vh-64px)]">
            <div className="relative h-72 md:h-96 w-full">
                ...
                <div className="absolute top-6 left-6 right-6 z-30 flex justify-between items-start">
                    ...
                    <SmoothieLink ingredientId={fruit.id} baseName={fruit.name} translatedName={displayName} />
                </div>
                ...
                <div className="absolute bottom-6 left-6 right-6 z-20 text-white max-w-7xl mx-auto">
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                        <div className="flex flex-wrap gap-2 mb-3">
                            {fruit.benefit && (
                                <span className={`px-3 py-1 text-xs font-bold rounded-full bg-${fruit.color || 'purple'}-500/80 backdrop-blur border text-white uppercase tracking-wider`}>
                                    {displayBenefit}
                                </span>
                            )}
                            {fruit.benefits && fruit.benefits.map(b => (
                                <span key={b} className={`px-3 py-1 text-xs font-bold rounded-full bg-${fruit.color || 'purple'}-500/80 backdrop-blur border text-white uppercase tracking-wider`}>
                                    {b}
                                </span>
                            ))}
                        </div>
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
                <div className="grid md:grid-cols-3 gap-8">
                    <div className="md:col-span-2 space-y-8">
                        {fruit.mainBenefits && (
                            ...
                            >
                                <h2 className="text-2xl font-bold mb-6 flex items-center gap-2">
                                    <Target className={`text-${fruit.color || 'purple'}-500`} />
                                    {t('detail.mainBenefits')}
                                </h2>
                                <div className="space-y-6">
                                    {displayMainBenefits && displayMainBenefits.map((benefit, i) => (
                                        <div key={i} className="flex gap-4">
                                            <div className={`mt-1 bg-${fruit.color || 'purple'}-100 dark:bg-${fruit.color || 'purple'}-900/30 p-2 rounded-xl h-fit`}>
                                                <CheckCircle2 className={`w-5 h-5 text-${fruit.color || 'purple'}-600 dark:text-${fruit.color || 'purple'}-400`} />
                                            </div>
                                            <div>
                        ...
                        {fruit.tips && (
                            <motion.section
                                ...
                                className={`bg-${fruit.color || 'purple'}-50 dark:bg-${fruit.color || 'purple'}-900/10 rounded-3xl p-6 md:p-8 border border-${fruit.color || 'purple'}-100 dark:border-${fruit.color || 'purple'}-900/30`}
                            >
                                <h2 className={`text-xl font-bold mb-3 flex items-center gap-2 text-${fruit.color || 'purple'}-800 dark:text-${fruit.color || 'purple'}-400`}>
                                    <Sparkles className="w-5 h-5" />
                                    {t('detail.dailyTip')}
                                </h2>
                                <p className={`text-${fruit.color || 'purple'}-900 dark:text-${fruit.color || 'purple'}-200/80 leading-relaxed`}>
                    ...
                    <div className="space-y-6">
                        {fruit.nutrients && fruit.nutrients.length > 0 && (
                            <motion.section
                                initial={{ opacity: 0, x: 20 }}
                                animate={{ opacity: 1, x: 0 }}
