...
export default function Smoothies() {
    ...
    useEffect(() => {
        if (location.state?.filterIngredient) {
            setActiveFilter(location.state.filterIngredient);
        }
    }, [location.state]);
    ...
    const filtered = smoothies.filter(s => {
        ...
        if (query.trim()) {
            matchesQuery = ingredients?.some(ing => normalize(ing).includes(normalize(query)));
        }

        if (activeFilter) {
            matchesActiveFilter = ingredients?.some(ing => normalize(ing).includes(normalize(activeFilter)));
        }
    ...
    const clearActiveFilter = () => {
        setActiveFilter('');
        // To handle edge case where user clicks 'X' but location.state still has it on reload
