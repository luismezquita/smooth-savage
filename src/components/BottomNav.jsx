        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 bottom-nav backdrop-blur-md border-t border-gray-100/10 dark:border-gray-800 pb-safe z-50">
            <nav className="flex justify-around items-center h-16 px-4">
                {navItems.map(({ id, path, label, Icon }) => {
                    ...
                    // Specific logic for smoothies to execute replace layout refresh
                    if (id === 'smoothies') {
                        return (
                            <button
                                key={id}
                                onClick={() => {
                                    scrollToTop();
                                    navigate(path, { replace: true, state: { resetSearch: true } });
                                }}
                                className={`nav-item flex flex-col items-center gap-1 ${isActive ? 'active' : ''} hover:text-[#ffcc00] opacity-90 hover:opacity-100`}
                            >
